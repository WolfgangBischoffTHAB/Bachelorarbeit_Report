...
\gls{SIMD} (Single Instruction Multiple Data) erlaubt es mehr als ein Datum gleichzeitig durch eine einzige Instruktion verarbeiten zu können. Die Addition zweier Vektoren geschieht aus mathematischer Sicht elementweise und jedes Elementpaar kann unabhängig von allen anderen Paaren in jeglicher Reihenfolge addiert werden. Damit eignet sich die Vektoraddition für \gls{SIMD} Anweisungen. Dazu muss die CPU die passende Hardware bereitstellen. Dann ist es möglich, dass die CPU wenn eine Vektoradditionsanweisung augeführt wird, gleich mehrere Elemente auf einmal miteiander addiert.

Abbildung \ref{fig:sisd_simd} stellt SISD auf der linken und SIMD auf der rechten Seite schematisch dar. SISD addiert pro Aufruf ein Elementpaar wobei SIMD mehrere Paares pro Aufruf addiert.
...
Eine RISC-V CPU besitzt die sogenannte X-Register-File. Es handelt sich damit um alle Register, die eine RISC-V CPU standardmäßig, ohne Extensions hat. Dies könnten beispielsweise die Register x0 bis x31 sein. Die Länge der Register in bits wird durch die Konstante XLEN festgelegt.

Die \gls{RVV} erweitert eine RISC-V CPU um zusätzliche Vektor Register v0 bis v31. Die Länge der Vektor Register wird durch die Konstante \gls{VLEN} (in Anlehnung an die Konstante \gls{XLEN}) vorgegeben. \gls{VLEN} wird bei der Synthese des Designs auf einen Wert festgelegt und ist nicht zur Laufzeit änderbar. \gls{VLEN} stellt somit eine Eigenschaft der CPU dar. Die \gls{RVV} Spezifikation \cite{rvv_specification} macht die Vorgabe:
...
\begin{enumerate}
    ...
    \item Die Anwendung kennt jetzt die Vektor Length (VL), die für diese Iteration gilt. Damit kennt die Software die Batch-Size. Die Software wird nun die Vector-Load Instructionen (vle8, vle16, ...) verwenden um einen Anteil des Vektors oder der Vektoren, der genau der Vektor Length (VL) entspricht in die Vektorregister zu laden. Die Anwendung wird auch einen Zeiger nach vorne verschieben, um sich zu merken von welcher Stelle in der nächsten Iteration geladen werden muss.
...
Zunächst wird in der linken Spalte der Abbildung \ref{fig:Matrix-Extension-Optionen} gezeigt, dass sich eine Matrix theoretisch elementweise berechnen lässt, wenn für jedes Element der Matrix individuel ein Skalarprodukt mit Hilfe der \gls{RVV}-Extension berechnet wird. Dies ist nur möglich, wenn die CPU die \gls{RVV} besitzt und gleichzeitig, wenn die Matrix spalten- oder zweilenweise in ein Vektorregister passt.
...
Bei der herkömmlichen Matrixmultiplikation über das Skalarpoduct von Zeilen und Spalten werden die beiden Matrizen A und B in ihrer kompletten Dimension betrachtet, da jeweils komplette Zeilen und Spalten verrechnet werden müssen um ein Matrix-Element zu bestimmen.

Bei sehr großen Matrizen ist dies nicht zielführend, da die CPU die Matrix nur schrittweise erfassen und damit nur sehr ineffizient berechnen kann.

Die angestrebte Verbesserung besteht darin, eine Matrix blockweise zu berechnen. Die Abbildung \ref{fig:matrix_tiling} visualisiert das blockweiße Vorgehen.
...
Die Matrizen muss nicht mehr im Ganzen in den CPU oder Cache passen sondern es kann nur ein Teil der Matrizen betrachtet werden.
...
Nachdem das Outer-Produkt definiert ist, lässt sich das Outer-Produkt aufaddieren, um die Matrixmultiplikation zu beschreiben.
...
Die Kombination aus dem blockweisen Ansatz und dem Outer Produkt mittels \gls{MAC} ermöglicht es ein Design zu erstellen, dass dem angestrebten RISC-V \gls{AME} Ansatz entspricht.
...
Das Kapitel wurde mit der Beschreibung der blockweisen Berechnung der Matrixmultiplikation unter Verwendung des Outer-Produkt Ansatzen geschlossen. Damit ist das Strip-Mining auch für Matrizen möglich.
