...
Für die Berechnung von Vektoren wurde die RISC-V Vector Extension (\gls{RVV}) \cite{rvv_specification} erstellt und ratifiziert. Die Vektor Extension ist für diese Arbeit relevant, da sie zwar Vektoren verarbeitet aber damit die Grundlage für die Verarbeitung von Matrizen legt.
...
Bei sehr großen Matrizen ist dies nicht zielführend, da die CPU die Matrix nur schrittweise erfassen und damit nur sehr ineffizient berechnen kann. Die angestrebte Verbesserung besteht darin, eine Matrix blockweise zu berechnen. Die Abbildung \ref{fig:matrix_tiling} visualisiert das blockweiße Vorgehen.
...
Die Kombination aus dem blockweisen Ansatz und dem Outer Product mittels \gls{MAC} ermöglicht es ein Design zu erstellen, dass dem angestrebten RISC-V \gls{AME} Ansatz entspricht.
