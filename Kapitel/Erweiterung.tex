...
Um die Matrixmultiplikation durchzuführen sollen neue Instruktionen hinzugefügt werden, die es noch nicht bereits in anderen Extensions für RISC-V gibt. Die Instruktionen sollen das Strip-Mining ermöglichen und sich damit an der \gls{RVV}-Extension orientieren.
...
Diese beiden Zustände sind sehr simple. Im Request--State wird das Signal matrix\_mul\_o aktiviert und den Response--State übergegangen. Im Response--State wird auf das Signal matrix\_operation\_wait\_i gewartet und dann die Matrixmultiplikation deaktiviert.
...
Der neue PROC\_RAM Prozess produziert Steuersignale für den RAM-Speicher der Matric-Engine. Abhängig von den Zählern, die in den Operationen Load und Store verändert werden, produziert der PROC\_RAM Prozess Signale, die die Load--Store--Unit verlassen und im CPU mit dem RAM für die Matrix--Engine verbunden sind. Damit wird der RAM der Matrix--Engine gesteuert und gibt entweder Daten für ein Load zurück oder speichert Daten für eine Store--Anweisung. Die Daten kommen aus dem Daten--Speicher und durchlaufen den Cache.
...
Wenn die LSU Daten über den Bus aus dem RAM bzw. dem Cache erhält, dann wird gleichzeitig dazu das entsprechende Element im Register geschrieben oder gelesen. Die Synchronisation der LSU mit dem PROC\_RAM Prozess geschieht über die Signale hail und tail.

Der zweite Teil der Matrix-Engine besteht aus der Berechnung des Outer-Product. Dazu sind bei 16 Elementen pro Matrixregister und damit mit 4x4 Matrizen vier Schritte notwendig. In den vier Schritten werden Zeilen aus der Matrix A und Spalten aus der Matrix B miteinander verrechnet. Mit vier Zeile und vier Spalten müssen also vier Berechnungen ausgeführt werden.
