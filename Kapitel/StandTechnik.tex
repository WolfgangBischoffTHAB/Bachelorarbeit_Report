...
In der RISC-V Vector Extension Spezifikation \cite{rvv_specification} wird das Konzept des Strip-Mining beschrieben und hervorgehoben. Eine grafische Darstellung ist in Abbildung \ref{fig:strip_mining_colfax} aus \cite{Colfax_Optimization_Techniques_2_3} zu sehen.

\begin{figure}[h]
\centering
\includegraphics[scale=0.35]{"strip_mining_2.png"}
\caption[Strip Mining]{Strip Mining (Quelle: \cite{Colfax_Optimization_Techniques_2_3}, S.10)}
\label{fig:strip_mining_colfax}
\end{figure}

Die Software lädt den Cache mit einem Teil einer oder mehrerer Vektoren voll und beginnt den Cache dann abzuarbeiten. Dabei wird ein Teil der Vektoren dann wiederum teilweise aus dem Cache in die CPU geladen, verarbeitet und danach wieder aus der CPU zurück in den Cache bzw. den Speicher übertragen. In Abbildung \ref{fig:strip_mining_colfax} aus \cite{Colfax_Optimization_Techniques_2_3} wird einer dieser Schritte als ``vector iteration'' bezeichnet.
...
Die Onlinezeitung Heise berichtet jüngst über die Strategien, die die großen Player der Chip-Industrie bezüglich Matrix-Rechenwerken verfolgen um KI auf Ihren CPUs effizient ausführbar zu machen \cite{heise_matrix_rechenwerke}. Apple, als Vorreiter, hat bereits Matrixrechenwerke in die M-Prozessoren integriert. Die Firma Qualcomm möchte diesen Schritt im Jahre 2026 vollziehen, während AMD und Intel die Erweiterung um eine Matrixengine im Jahre 2027 angehen.

In dem Heise-Bericht \cite{heise_matrix_rechenwerke} wird angesprochen, dass die grundlegende Operation der Matrixrechenwerke die Matrix-Matrix-Operation ist, also die Multiplikation von zwei Matrizen. Insbesondere sollen von den Matrixrechenwerken die speziellen, neuartigen Datentypen unterstützt werden, die von modernen Neuronalen Netzen verwendet werden und die sich von den traditionellen Datentypen herkömmlicher CPU-Designs unterscheiden.
...
Die NEORV32 CPU besitzt im Design vorgesehene Erweiterungspunkte namens CFS und CFU. Das CFS (Custom Functions Subsystem) erlaubt es Hardware-Beschleuniger in die NEORV32 CPU einzubringen, die über Speicheradressen (Memory Mapping) aus einer Softwareanwendung mit Daten versorgt werden können. CFU (Custom Function Unit) erlaubt es einzelne Instruktionen zu ergänzen um die CPU um kleinschrittige Operationen zu erweitern.
