...
Im Rahmen dieser Arbeit wurde der existierende NEORV32 CPU um Komponenten zur Matrix-Berechnung erweitert. Die Komponenten unterliegen starken Einschränkungen. Unter anderem können zum jetzigen Zeitpunkt lediglich 4x4 Matrizen mit vorzeichenlosen 32-Bit Zahlen berechnet werden.

Die Erweiterung orientiert sich an der Befehlsmenge der bereits existierenden RISC-V Vektor-Extension und greift damit auch das Konzept des Strip-Mining auf. Strip-Mining erlaubt die Erstellung eines Hardware-Software-Co-Designs, welches in der Lage ist Matrixmultiplikationen auch größere Matrizen durch Multiplikationen kleinerer Dimensionen zusammenzusetzen.
...
Die Alternative wäre es, ein neues Design zu beginnen welches dann die neuen Anforderungen ermöglicht. Ein Beispiel für einen neue Anforderung ist es beispielsweise mehr als einen Speicher-Zugriff parallel ausführen zu können. Die Hardware für die Outer-Produkt Berechnung könnte dann mehrmals synthetisiert werden. Jede Instanz der Outer-Produkt Berechnung könnte möglicherweise über ihren eigenen Zugriff auf den Speicher verfügen. Wenn der Speicher echt-parallel Daten liefern kann, wird eine Matrix-Berechnung parallel statt sequentiel ausgeführt.
...
Als nächste Schritte zur Fortführung dieser Arbeit ist es notwendig die Matrix-Extension intelligenter und damit performanter zu gestalten. Der Vergleich mit anderen Designs wie z.B. \cite{DBLP:journals/corr/abs-2104-03142} und \cite{scoss} führt zu neuen Erkenntnissen und einen Zuwachs an Erfahrung.
