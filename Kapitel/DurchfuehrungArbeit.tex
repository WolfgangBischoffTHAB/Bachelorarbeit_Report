...
Die Simulation verwendet Signale, die das Design ``stimulieren''. Dabei werden die Signale so gewählt, dass ein bestimmter Ablauf provoziert wird. Dieser Ablauf kann dann durch die Simulation getestet werden. Das Design verarbeitet die eingehenden Signale und produziert Ausgabesignale. Der Simulator speichert alle Signaländerungen, sowohl die Eingaben an das Design, als auch dessen Ausgaben, in Value Change Dump (VCD)-Dateien. Diese VCD-Dateien können dann in einem separaten Schritt ausgewertet werden.

Ein sogenannter Waveform-Viewer kann die VCD-Dateien laden und die Signale dann anzeigen, wie es in \ref{fig:sample_waveform} dargestellt ist.
